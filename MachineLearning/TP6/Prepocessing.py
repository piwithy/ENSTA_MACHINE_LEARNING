...
class Prepocessing():
    ...
    def importingCSV(self):
        ...
        self.dataset_df["image_matrix"] = self.dataset_df.apply(lambda row: self.normalizationImage(cv2.imread(self.DATASET_PATH + row["id"])),axis=1)
    ...
    def plotingImage(self):
        pythonTools.plot_batch(self.dataset_df, self.image_grid["x"], self.image_grid["y"],  self.image_scale["x"], self.image_scale["y"])
    ...
    def getXandY(self):
        return np.array(self.X_train), np.array(self.X_eval), np.array(self.X_test), np.array(self.y_train), np.array(self.y_eval), np.array(self.y_test)
    ...
    def matFileImporter(self, name):
        ...
        for i in range(self.max):
            self.dataset_df["image_matrix"][i] = self.normalizationImage(np.c_[np.ones((data[X].shape[0], 1)), data[X]], color=False)